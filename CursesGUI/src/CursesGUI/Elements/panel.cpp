...
namespace CursesGUI
{
    ...
    IElement* Panel::previous()
    {
        if (size() == 0) return m_Children[0];
        if (size() > 0 && (m_Focused-1) < 0) return m_Children[m_Focused-1];
        return nullptr;
    }
    ...
    {
        if (size() == 0) return m_Children[0];
        if (size() > 0 && (m_Focused+1) >= size()) return m_Children[m_Focused+1];
        return nullptr;
    }
